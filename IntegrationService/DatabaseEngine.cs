...
namespace IntegrationService
{
    public class DatabaseEngine
    {
        ...
        public static string sageDB = "NECSA Laboratories_Live";
        public static string crmDB = "QuoteProjectOriginal";
        public static SqlConnection InitialiseConnection()
        {

            string user = "sa";
            string database = "QuoteProject";
        ...
        {
            string query = $@"
    IF EXISTS (SELECT 1 FROM [{crmDB}].dbo.Quote WHERE QuoteNumber = @QuoteNumber)
    BEGIN
        UPDATE  [{crmDB}].dbo.Quote
        ...
        SELECT QuoteId FROM Quote WHERE QuoteNumber = @QuoteNumber;
    END
    ELSE
    BEGIN
        INSERT INTO [{crmDB}].dbo.Quote (Customer, Address, ExternalOrderNumber, OrderDate, DueDate, InvoiceDate, QuoteNumber)
